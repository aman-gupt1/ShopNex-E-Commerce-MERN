...
export const updateProfile = handleAsyncError(async (req, res, next) => {
  ...
  if (avatar) { // only proceed if avatar exists
    const user = await User.findById(req.user.id);
...
export const getSingleUser=handleAsyncError(async(req,res,next)=>{
    ...
    if(!user){
        return next(new HandleError(`User doesn't exist with this id: ${req.params.id.trim()}`,400))
    }
    res.status(200).json({
        success: true,
        user
...
//     const {role}=req.body.trim();
//     const newUserData={
//         role
//     }
//     const user=await User.findByIdAndUpdate(req.params.id.trim(),newUserData,{
//         new:true,
//         runValidators:true
//     })
//     if(!user){
...
//   const Delete=req.params.id.trim();
//    const user =await User.findById(Delete);
//    if(!user){
//     return next(new HandleError("User doesn't exist",400))
//    }
//    const imageId=user.avatar.public_id;
